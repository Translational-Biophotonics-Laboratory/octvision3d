...
def segnrrd2nnUNet(path):
    ...
    # Dictionary mapping segmentation labels to their corresponding
    # numeric values
    labels_dict = {
        ...
        "VIT": 10, # Vitreous
        "HYA": 11, # Hyaloid
        "SHS": 12, # Subretinal Hyaloid Space 
        "ART": 13, # Artifacts
        "ERM": 14, # Epiretinal Membrane
        "SES": 15  # Sub-ERM Space
    }

    # Retrieve paths for TIFF volumes and segmentation NRRD files, excluding those with
    # "slo" in their names
    ...
    for vol_path, seg_path in zip(vol_paths, seg_paths):
        ...
        # Convert one-hot encoded bitmap to label array, flipping axes from (X, Y, Z)
        # to (Z, Y, X)
        ...
        save_json(
            ...
            os.path.join(imagesTr, f"{vol_name}.json")
        )
        save_json(
            {"spacing": [81.0, 1.0, 2.9]},
            os.path.join(labelsTr, f"{seg_name}.json")
        )
